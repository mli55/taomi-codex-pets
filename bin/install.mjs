...
const baseId=pet==='nono'?`seer-${variant}-nono${nonoColor==='original'?'':'-'+nonoColor}`:`mole-ram-${form}-${colorSlugs[+color]}`;
const displayName=pet==='nono'?({normal:'普通 NoNo',super:'超能 NoNo',annual:'至尊 NoNo'}[variant])+(nonoColor==='original'?'':' · '+nonoPalette.find(c=>c.id===nonoColor).name):forms[form]+' · '+colorNames[+color];
try{
