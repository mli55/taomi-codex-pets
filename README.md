...
三个版本均展示九种 Codex 状态。普通 NoNo 的等待／工作使用惊讶／充电，悬停使用生气；超能使用惊讶／玩魔方，悬停使用召唤旋转；至尊使用轻微皱眉／召唤后半段，悬停使用惊讶。三个版本的检查状态使用高兴；超能跳跃使用召唤旋转，打招呼使用生气片段。原始动作与逐帧选择以各 `*-animation-sources.json` 为准。
