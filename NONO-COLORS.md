...
换色范围依据官方 `RobotCoreDLL.swf` 的 `NonoModel`：方向动画为 `color_1` / `color_2`，动作动画为 `color_` 开头的子容器。使用原 SWF 的这些容器生成黑、白端点，再按 RGB 插值得到图集；眼睛、发光效果、道具等容器不整体染色。支持普通、超能和至尊 NoNo。至尊版部分动作未命名换色容器；按方向素材 `color_1` 中白色与暖灰色壳体的填充组合识别对应形状，保留乳白护边、黄色耳部和眼睛。这里的离线配色选项不表示所有颜色在原游戏各会员等级中都能取得。
