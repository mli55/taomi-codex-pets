...
p=argparse.ArgumentParser();p.add_argument('--ram-source',type=Path,required=True);p.add_argument('--nono-source',type=Path,required=True);p.add_argument('--ffdec',required=True);p.add_argument('--assets',type=Path,required=True);p.add_argument('--work',type=Path,required=True);a=p.parse_args();a.work.mkdir(parents=True,exist_ok=True)
plan=json.loads(Path(__file__).with_name('selected-pet-clips.json').read_text());ram=json.loads((a.ram_source/'original-actions.json').read_text());nono=json.loads((a.nono_source/'candidates.json').read_text());jobs=[];ramrows={};nonorows={}
for form,states in plan['ram'].items():
 ...
 for state,selection in states.items():
  ...
  entry=next(x for x in info['rows'] if x['state']==source);action=next(x for x in info['actions'] if x['label']==entry['gameAction']);selected[state]={**entry,'state':state,'actionFrames':frames,'frames':[action['start']+f-1 for f in frames],'sourceFrameCount':action['sourceFrameCount']}
...
for filename,forms in [('animation-sources.json',['super','water','wood','fire']),('ram-level-animation-sources.json',['junior','middle','senior','classic'])]:
 ...
 for form in forms:
  ...
  for state,entry in ramrows[form].items():
   ...
   pairs=[c.clean(Image.open(nd/f'{n}.png'),Image.open(pd/f'{n}.png')) for n in entry['frames']];c.pack_row(pairs,row,atlas,mask)
