...
当前精确选帧和行分配由 `scripts/selected-pet-clips.json` 记录。`reselect-pet-clips.py` 从原素材逐帧导出，舞蹈、高兴和生气优先收回正面姿态，部分短片段倒序回到起始帧；帧数与播放时长均遵循原客户端。每行统一缩放，避免远处道具挤小身体，拉姆工作行已移除玩球。转身行采用每行统一缩放，并限制原片舞台位移，防止大幅转身时角色整体缩小；此定位同时用于身体和配色遮罩。新选帧保留眼白排除规则；旧的定点眼白修复脚本针对旧帧号，不应用于重选后的图集。
...
超能 NoNo：打招呼使用 `super-exp-2.swf` 生气帧 14、18、26、30；跳跃使用 `super-action-6.swf` 召唤旋转帧 42、45、48、51、56；检查使用 `super-exp-1.swf` 高兴帧 89、90、92、94、92、89。眼睛来自原始完整帧，不做拼接、模糊或重画。网页关闭画布三倍放大的额外平滑插值，保留图集本身的抗锯齿像素。

至尊 NoNo：工作使用 `annual-action-2.swf` 原打招呼片段（53、55、57、59、61、64）；打招呼使用 `annual-exp-3.swf` 原跳跃片段（1、6、11、1）；跳跃使用 `annual-action-6.swf` 原工作旋转片段（45、48、50、59、64）。按各状态的 6／4／5 帧规格选取原始完整帧，其他六行保持不变。

普通和神力拉姆：工作沿用原跳跃的五个转身姿势，并以首帧收尾。跳跃使用原 `woter` 动作，选取落水、淋湿和恢复阶段，避开飞溅水滴远离身体或翻转导致角色缩小的部分。各等级与元素的准确帧号见选帧计划和来源记录。

超级拉姆工作：基础形态使用原 `dance` 帧 1、10、19、28、36、1；神力形态使用 19、28、37、46、37、19。选择前段轻微摇摆，与悬停后段转身区分；整帧保留原始图形。
