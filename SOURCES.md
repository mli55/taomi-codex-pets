...
普通和神力拉姆：工作从原转身片段连续取六个角度，不重复首尾正面姿势。跳跃使用原 `woter` 动作，选取靠近身体的淋湿阶段，不额外追加恢复帧；末帧保留滴水或原片自然收尾，避开飞溅水滴远离身体或翻转导致角色缩小的部分。各等级与元素的准确帧号见选帧计划和来源记录。

超级拉姆工作：基础形态使用原 `dance` 帧 10、19、28、36、28、19；神力形态使用 19、28、37、46、37、28。选择前段轻微摇摆，与悬停后段转身区分；整帧保留原始图形。

woter：以此前未遮挡的身体遮罩为参考，整段身体宽度按待机的 90% 定比例；水花不参与缩放。各帧以原始身体遮罩中心对齐，保留原图形及自然形变。超过单元格的水花会触发导出错误，必须重新选帧；不自动缩小身体。循环调整将该比例和定位记录在 `registration.scale` 与 `registration.anchor`，换帧时保持不变。

循环衔接检查覆盖全部 11 个形态的九种状态。53 行重排已有像素，11 行补入原素材中间姿势；待机和左右移动保持原样。转圈沿原方向完成一圈，表情、摇摆以邻近姿势收尾。客户端三遍播放及末帧时长不变；不把首帧再放在末尾制造额外停顿。新增姿势保留此前比例，NoNo 同时保留原舞台坐标；准确选帧与固定布局见各来源记录。
