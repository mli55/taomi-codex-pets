...
普通和至尊 NoNo 待机固定身体：`stabilize-nono-idle.py` 保留第一帧身体和屏幕高光，仅提取同版本原待机素材中的眼睛。超能 NoNo 已取消此处理，直接选用原始正面完整帧 1、2、3、3、2、1；不拼接眼睛、耳朵或身体，保留原动作的小幅变化。节奏仍为客户端规定的 6.6 秒。

超能 NoNo：打招呼改为 `super-exp-2.swf` 生气帧 14、18、26、30；检查改为 `super-action-6.swf` 召唤旋转帧 42、45、48、51、54、56。其余七行逐像素保留，待机另按上述原帧方案处理。
