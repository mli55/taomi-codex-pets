...
网站与安装命令的配色编号按展示顺序：1 黄色、2 红色，3–10 不变。`palette.json` 的 `gameColorId` 保留上表原游戏 ID；仅编号调整，RGB 配色不变。
...
普通版召唤、魔方以及至尊版充电资源请求返回 404。当前普通版工作状态使用充电，超能版使用玩魔方，至尊版使用开机后半段；普通版悬停使用生气，超能版使用召唤旋转，至尊版使用召唤旋转，至尊版等待状态使用自身生气动作的轻微表情片段。所有动作都来自对应版本，没有混用外形。两种版本的帧映射分别见 `nono-normal-animation-sources.json` 和 `nono-annual-animation-sources.json`。原 SWF 中一帧外壳容器延长至 63 帧以渲染其原有嵌套方向动画，不改变原始图形或颜色。
...
此前按主时间轴的短标签区间取帧，会只得到嵌套长动作的开头。现在先展开完整嵌套时间轴并导出覆盖全程的候选姿势，再选取可读的短片段；不是将整个长动作等间隔压缩到几张图片。普通和神力拉姆工作状态使用此前悬停的舞蹈转身片段，悬停改用原 `woter` 动作；超级拉姆使用跳舞前段的摇摆工作、后段的转身悬停。检查状态使用高兴。
...
当前精确选帧和行分配由 `scripts/selected-pet-clips.json` 记录。`reselect-pet-clips.py` 从原素材逐帧导出，转圈按原方向连续取样；摇摆和表情选择能接回首帧的相邻姿势，不再统一以恢复帧结尾；帧数与播放时长均遵循原客户端。每行统一缩放，避免远处道具挤小身体，拉姆工作行已移除玩球。转身行采用每行统一缩放，并限制原片舞台位移，防止大幅转身时角色整体缩小；此定位同时用于身体和配色遮罩。新选帧保留眼白排除规则；旧的定点眼白修复脚本针对旧帧号，不应用于重选后的图集。

三种 NoNo 待机均直接选用原始完整帧，不拼接眼睛、耳朵或身体，不固定身体。普通选 5、3、1、3、5、5；超能选 1、2、3、3、2、1；至尊选 1、3、5、7、5、1。保留原动作的小幅变化，节奏仍为客户端规定的 6.6 秒。已移除眼睛拼接脚本，避免重建时再次应用。

超能 NoNo：打招呼使用 `super-exp-2.swf` 生气帧 14、18、26、30；跳跃使用 `super-action-6.swf` 召唤旋转帧 42、45、48、51、53；检查使用 `super-exp-1.swf` 高兴帧 89、90、92、94、92、90。眼睛来自原始完整帧，不做拼接、模糊或重画。网页关闭画布三倍放大的额外平滑插值，保留图集本身的抗锯齿像素。

至尊 NoNo：工作使用 `annual-action-2.swf` 原打招呼片段（53、55、57、59、57、55）；打招呼使用 `annual-exp-3.swf` 原跳跃片段（1、6、11、6）；跳跃使用 `annual-action-6.swf` 原工作旋转片段（45、49、53、57、61）。按各状态的 6／4／5 帧规格选取原始完整帧。

普通和神力拉姆：工作从原转身片段连续取六个角度，不重复首尾正面姿势。跳跃使用原 `woter` 动作，恢复最初的水珠落下、淋湿和恢复选帧，避开飞溅水滴远离身体或翻转导致角色缩小的部分。各等级与元素的准确帧号见选帧计划和来源记录。

超级拉姆工作：基础形态使用原 `dance` 帧 10、19、28、36、28、19；神力形态使用 19、28、37、46、37、28。选择前段轻微摇摆，与悬停后段转身区分；整帧保留原始图形。

woter 已回退到 `5507c07` 的图集行、选帧和整体缩放，保留水珠砸下来的过程；撤回按身体定比例及后续水滴循环重排。其他动作的循环优化保留。

循环衔接检查覆盖全部 11 个形态的九种状态。47 行重排已有像素，11 行补入原素材中间姿势；待机和左右移动保持原样。转圈沿原方向完成一圈，表情、摇摆以邻近姿势收尾。客户端三遍播放及末帧时长不变；除恢复原版的 woter 外，避免重复首帧制造额外停顿。新增姿势保留此前比例，NoNo 同时保留原舞台坐标；准确选帧与固定布局见各来源记录。

基础超级拉姆失败使用原 `angry` 帧 6、9、12、16、21、24、18、12，全程保持生气，移除正常表情的过渡帧。保留该行原有比例与定位。

NoNo 动作尺寸：普通的打招呼、跳跃、失败、工作、检查，以及超能的打招呼、失败、等待、检查，使用原始完整帧重新导出。以各自待机的脸部屏幕宽度为参照，整行使用同一比例；保留原始表情、帧序与自然形变，不逐帧缩放或拼接眼睛。`registration.scale`、`sourceBox` 和 `cellOffset` 记录比例与定位，彩色图集沿用同一布局。至尊各动作未发现同样的放大差异，保持原样。

基础超级拉姆的 failed 改用原 `spring` 动作落地后沮丧的片段（137、139、141、143、145、143、139、137 帧），避免原 angry 的表情变化过弱和后段背对镜头。该行统一缩放并限制上下位移，保持身体清楚可见；神力超级拉姆的 failed 和其他行不变。
